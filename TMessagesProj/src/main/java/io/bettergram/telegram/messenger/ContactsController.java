...
public class ContactsController {
    ...
    public void checkAppAccount() {
        ...
        try {
            Account[] accounts = am.getAccountsByType("io.bettergram.telegram.messenger");
            systemAccount = null;
        ...
        if (UserConfig.getInstance(currentAccount).isClientActivated()) {
            ...
            if (systemAccount == null) {
                try {
                    systemAccount = new Account("" + UserConfig.getInstance(currentAccount).getClientUserId(), "io.bettergram.telegram.messenger");
        ...
        Utilities.globalQueue.postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                /*if (schedule) {
                    try {
                        AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
                        Account[] accounts = am.getAccountsByType("io.bettergram.telegram.account");
                        boolean recreateAccount = false;
                        if (UserConfig.getInstance(currentAccount).isClientActivated()) {
                            if (accounts.length != 1) {
                                FileLog.e("detected account deletion!");
                                currentAccount = new Account(UserConfig.getInstance(currentAccount).getCurrentUser().phone, "io.bettergram.telegram.account");
...
//        query.add(builder.build());
        ...
        builder.withValue(ContactsContract.Data.MIMETYPE, "vnd.android.cursor.item/vnd.io.bettergram.telegram.messenger.android.profile");
